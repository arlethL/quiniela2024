document.addEventListener("DOMContentLoaded", function() {
    ...
    const partidosPorJornada = {
        ...
        5: ["Querétaro vs Cruz Azul", "Mazatlán vs Pachuca", "Tijuana vs Rayados", "Necaxa vs Juárez FC", "León vs Santos", "Atlas vs Pumas", "Tigres vs Chivas","America vs puebla", "Toluca vs Atlético de San Luis"],        6: ["Atlético de San Luis vs Atlas", "Mazatlán vs Puebla", "Tijuana vs León", "Pachuca vs Querétaro", "Chivas vs Juárez FC", "Rayados vs Toluca", "Cruz Azul vs América", "Pumas vs Tigres", "Santos vs Necaxa"],
    ...
    function actualizarResultados() {
        ...
        const resultadosGuardados = JSON.parse(localStorage.getItem('resultadosGuardados'));
        if (resultadosGuardados) {
            partidos.forEach((partido, index) => {
                const selector = document.querySelector(`select[name="partido${index + 1}"]`);
                if (selector) {
                    selector.value = resultadosGuardados[`partido${index + 1}`] || "0";
                }
            });
        }
    ...
    function ordenarPorPuntos() {
        const rows = Array.from(tablaQuinielasBody.rows);
        rows.sort((a, b) => {
        ...
        rows.forEach(row => tablaQuinielasBody.appendChild(row));
    }
    ...
    cargarQuinielasButton.addEventListener("click", function() {
        ...
        .then(data => {
            ...
            data.forEach(quiniela => {
                ...
                eliminarButton.addEventListener('click', () => eliminarQuiniela(quiniela._id));
                ...
                row.insertCell().textContent = quiniela.puntos || 0; // Mostrar puntos guardados
            });
            ordenarPorPuntos(); // Ordenar la tabla después de cargar los datos
        })
        .catch(error => {
    ...
    calcularPuntosForm.addEventListener("submit", function(e) {
        ...
        // Guardar los resultados seleccionados en localStorage
        localStorage.setItem('resultadosGuardados', JSON.stringify(resultados));
        ...
        .then(data => {
            ...
            data.forEach(({ nombre, puntos }) => {
                const row = Array.from(tablaQuinielasBody.rows).find(row => row.cells[1].textContent === nombre);
                if (row) {
                    row.cells[row.cells.length - 1].textContent = puntos;
                }
            });
            ordenarPorPuntos(); // Ordenar la tabla después de actualizar los puntajes
        })
        .catch(error => {
            console.error('Error:', error);
        });
    ...
    exportarExcelButton.addEventListener("click", function() {
        const jornada = jornadaSelect.value;
        window.location.href = `/api/quiniela/exportar-excel?jornada=${jornada}`;
    });

    function eliminarQuiniela(id) {
        if (confirm('¿Estás seguro de que deseas eliminar esta quiniela?')) {
            fetch(`/api/quiniela/usuario/${id}`, {
                method: 'DELETE'
            })
            .then(response => {
                if (response.ok) {
                    // Remover la fila de la tabla
                    const row = Array.from(tablaQuinielasBody.rows).find(row => row.cells[0].textContent === id);
                    if (row) {
                        tablaQuinielasBody.removeChild(row);
                    }
                } else {
                    console.error('Error al eliminar la quiniela');
                }
            })
            .catch(error => {
                console.error('Error:', error);
